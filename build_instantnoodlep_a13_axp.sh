...
export HOME="/home/droidme" && echo "home set to $HOME" >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log
export BDEVICE="instantnoodlep"
...
#gpg-agent --homedir /home/droidme/.gnupg --use-standard-socket --daemon
gpg --list-keys

cd /usr/src/android/axp/Build/LineageOS-20.0
source build/envsetup.sh >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log 2>&1 && echo "sourcing envsetup: success" >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log
source /usr/src/android/axp/Scripts/init.sh >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log 2>&1 && echo "sourcing init: success" >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log

# support A14 and later


# ensure vendor secpatch date is set
source vendor/firmware/instantnoodlep/0SOURCE || true
        
breakfast lineage_instantnoodlep${brf_suffix}-user
...
if [ "1" -eq 1 ];then
    ...
    mkdir -p /usr/src/android/ccache/a13
    export CCACHE_DIR="/usr/src/android/ccache/a13"
...
f_detect_kerr(){
    set +e
    resetEnv
    vendor/axp/scripts/find_cve_error.sh --parse -k kernel/oneplus/sm8250 -L /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log
    exit 5
}
    ...
    echo "[GIT] checking kernel changes in kernel/oneplus/sm8250"
    ...
    if [ $CMT -eq 1 -o $CMTL -gt 0 ];then 
       ...
       echo "[GIT] committed scripted kernel changes for kernel/oneplus/sm8250" >> /home/droidme/build_instantnoodlep_axp_2025-07-05.1.log
    fi
    git status
    croot
    ...
    resetEnv
        mka generate_verity_key
            buildDevice instantnoodlep || f_detect_kerr
    ...
    echo -e "\t${FPID:32:16}"
    echo -e "\t${FPID:48:16}\n"
    out/host/linux-x86/bin/avbtool info_image --image out/target/product/instantnoodlep/vbmeta.img 2> /dev/null || true
    out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/instantnoodlep/vbmeta.img 2> /dev/null \
        || (out/host/linux-x86/bin/avbtool info_image --image out/target/product/instantnoodlep/obj/PACKAGING/target_files_intermediates/lineage_instantnoodlep-target_files-eng.emy/IMAGES/vbmeta.img;\
            out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/instantnoodlep/obj/PACKAGING/target_files_intermediates/lineage_instantnoodlep-target_files-eng.emy/IMAGES/vbmeta.img)
    
    echo -e "\n\nSUCCESS:\nYou should find your build here:\n/usr/src/android/zips/instantnoodlep/axp/zips/instantnoodlep/LineageOS-20.0/release_keys/\n${ZIPINF}"
