...
export HOME="/home/jenkins" && echo "home set to $HOME" >> /home/jenkins/build_bluejay_a13_axp_13.0.80.log
export BDEVICE="bluejay"
...
source build/envsetup.sh >> /home/jenkins/build_bluejay_a13_axp_13.0.80.log 2>&1 && echo "sourcing envsetup: success" >> /home/jenkins/build_bluejay_a13_axp_13.0.80.log
source /ssd2/androidsource/do-not-touch/axp/Scripts/init.sh >> /home/jenkins/build_bluejay_a13_axp_13.0.80.log 2>&1 && echo "sourcing init: success" >> /home/jenkins/build_bluejay_a13_axp_13.0.80.log

breakfast lineage_bluejay-user
...
# containing the official Google images + AXP.OS based on vendor/firmware/bluejay/0SOURCE
VERSION="20.0-20241218-dos"
source vendor/firmware/bluejay/0SOURCE
if [ -f out/bluejay-factory.zip ];then rm out/bluejay-factory.zip;fi
wget -q https://dl.google.com/dl/android/aosp/bluejay-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/bluejay-factory.zip || (wget https://dl.google.com/dl/android/aosp/bluejay-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/bluejay-factory.zip ; exit 9)
...
unzip /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0/out/bluejay-factory.zip -x "*/image-bluejay-*.zip" -d factory
cd factory/bluejay-${FACTORYID}
cp ../../AXP.OS-$VERSION-bluejay-fastboot.zip image-bluejay-${FACTORYID}.zip
...
cd ..
zip -r ../AXP.OS-${VERSION}-bluejay-factory.zip bluejay-${FACTORYID}
cd ..
sha256sum AXP.OS-${VERSION}-bluejay-factory.zip > AXP.OS-${VERSION}-bluejay-factory.zip.sha256
cd /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0
...
echo -e "\t${FPID:32:16}"
echo -e "\t${FPID:48:16}\n"
out/host/linux-x86/bin/avbtool info_image --image out/target/product/bluejay/vbmeta.img 2> /dev/null || true
out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/bluejay/vbmeta.img 2> /dev/null \
    || (out/host/linux-x86/bin/avbtool info_image --image out/target/product/bluejay/obj/PACKAGING/target_files_intermediates/lineage_bluejay-target_files-eng.emy/IMAGES/vbmeta.img;\
        out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/bluejay/obj/PACKAGING/target_files_intermediates/lineage_bluejay-target_files-eng.emy/IMAGES/vbmeta.img)

echo -e "\n\nSUCCESS: You should find your build here:\n/ssd/axp/zips/bluejay/LineageOS-20.0/release_keys/\n"
