...
export HOME="/home/jenkins" && echo "home set to $HOME" >> /home/jenkins/build_sargo_a13_axp_13.1.1.log
export BDEVICE="sargo"
...
source build/envsetup.sh >> /home/jenkins/build_sargo_a13_axp_13.1.1.log 2>&1 && echo "sourcing envsetup: success" >> /home/jenkins/build_sargo_a13_axp_13.1.1.log
source /ssd2/androidsource/do-not-touch/axp/Scripts/init.sh >> /home/jenkins/build_sargo_a13_axp_13.1.1.log 2>&1 && echo "sourcing init: success" >> /home/jenkins/build_sargo_a13_axp_13.1.1.log

# support A14 and later

        
breakfast lineage_sargo${brf_suffix}-user
...
if [ "1" -eq 1 ];then
    ...
    export EXTENDROM_PACKAGES="AOSmium_webview64 additional_repos.xml AuroraStore F-Droid noeSpeakNG noDefaultBrowserWebView Etar_ER FossifyGallery GsfProxy_GH Magisk MicrogGmsCore_GH NeoLauncher-latest Phonesky_AXP-OS OpenEUICC_AXP-OS"
    ...
    echo "[GIT] checking kernel changes in kernel/google/msm-4.9"
    ...
    if [ $CMT -eq 1 -o $CMTL -gt 0 ];then 
       ...
       echo "[GIT] committed scripted kernel changes for kernel/google/msm-4.9" >> /home/jenkins/build_sargo_a13_axp_13.1.1.log
    fi
    git status
    croot
...
# containing the official Google images + AXP.OS based on vendor/firmware/sargo/0SOURCE
VERSION="20.0-20250326-dos"
FZIP="AXP.OS-${VERSION}-sargo"
source vendor/firmware/sargo/0SOURCE
if [ -f out/sargo-factory.zip ];then rm out/sargo-factory.zip;fi
if [ -f "/home/androidsource/ZIPs/axp/persistent/sargo/sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip" ];then
    echo "Using previously downloaded factory image: sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip"
    cp "/home/androidsource/ZIPs/axp/persistent/sargo/sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip" out/sargo-factory.zip 
else
    echo "sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip not found in: /home/androidsource/ZIPs/axp/persistent/sargo, downloading.."
    wget -q https://dl.google.com/dl/android/aosp/sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/sargo-factory.zip || (wget https://dl.google.com/dl/android/aosp/sargo-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/sargo-factory.zip ; exit 9)
fi
...
unzip /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0/out/sargo-factory.zip -x "*/image-sargo-*.zip" -d factory
cd factory/sargo-${FACTORYID}
cp ../../${FZIP}-fastboot.zip image-sargo-${FACTORYID}.zip     
...
cd ..
zip -r ../${FZIP}-factory.zip sargo-${FACTORYID}
cd ..
sha512sum ${FZIP}-factory.zip > ${FZIP}-factory.zip.sha512
mv ${FZIP}-factory.zip* /home/androidsource/ZIPs/axp/zips/sargo/LineageOS-20.0/release_keys//target_files/
FZIPINF="\n\tand the FACTORY zip here: /home/androidsource/ZIPs/axp/zips/sargo/LineageOS-20.0/release_keys//target_files/${FZIP}-factory.zip\n"
cd /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0
    ...
    echo -e "\t${FPID:32:16}"
    echo -e "\t${FPID:48:16}\n"
    out/host/linux-x86/bin/avbtool info_image --image out/target/product/sargo/vbmeta.img 2> /dev/null || true
    out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/sargo/vbmeta.img 2> /dev/null \
        || (out/host/linux-x86/bin/avbtool info_image --image out/target/product/sargo/obj/PACKAGING/target_files_intermediates/lineage_sargo-target_files-eng.emy/IMAGES/vbmeta.img;\
            out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/sargo/obj/PACKAGING/target_files_intermediates/lineage_sargo-target_files-eng.emy/IMAGES/vbmeta.img)
    
    echo -e "\n\nSUCCESS:\nYou should find your build here:\n/home/androidsource/ZIPs/axp/zips/sargo/LineageOS-20.0/release_keys/\n${ZIPINF}"
