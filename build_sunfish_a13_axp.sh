...
export HOME="/home/jenkins" && echo "home set to $HOME" >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log
...
source build/envsetup.sh >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log 2>&1 && echo "sourcing envsetup: success" >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log
source /ssd2/androidsource/do-not-touch/axp/Scripts/init.sh >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log 2>&1 && echo "sourcing init: success" >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log

# support A14 and later

        
breakfast lineage_sunfish${brf_suffix}-user
...
echo "[GIT] checking kernel changes in kernel/google/msm-4.14"
...
if [ $CMT -eq 1 -o $CMTL -gt 0 ];then 
   ...
   echo "[GIT] committed scripted kernel changes for kernel/google/msm-4.14" >> /home/jenkins/build_sunfish_a13_axp_13.0.41.log
fi
git status
croot
...
# containing the official Google images + AXP.OS based on vendor/firmware/sunfish/0SOURCE
VERSION="20.0-20250204-SLIM"
FZIP="AXP.OS-${VERSION}-sunfish"
source vendor/firmware/sunfish/0SOURCE
if [ -f out/sunfish-factory.zip ];then rm out/sunfish-factory.zip;fi
if [ -f "/ssd/axp/persistent/sunfish/sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip" ];then
    echo "Using previously downloaded factory image: sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip"
    cp "/ssd/axp/persistent/sunfish/sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip" out/sunfish-factory.zip 
else
    echo "sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip not found in: /ssd/axp/persistent/sunfish, downloading.."
    wget -q https://dl.google.com/dl/android/aosp/sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/sunfish-factory.zip || (wget https://dl.google.com/dl/android/aosp/sunfish-${FACTORYID}-factory-${FACTORYIDHASH}.zip -O out/sunfish-factory.zip ; exit 9)
fi
...
unzip /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0/out/sunfish-factory.zip -x "*/image-sunfish-*.zip" -d factory
cd factory/sunfish-${FACTORYID}
cp ../../${FZIP}-fastboot.zip image-sunfish-${FACTORYID}.zip     
...
cd ..
zip -r ../${FZIP}-factory.zip sunfish-${FACTORYID}
cd ..
sha256sum ${FZIP}-factory.zip > ${FZIP}-factory.zip.sha256
mv ${FZIP}-factory.zip* /ssd/slim/zips/sunfish/LineageOS-20.0/release_keys//target_files/
FZIPINF="\n\tand the FACTORY zip here: /ssd/slim/zips/sunfish/LineageOS-20.0/release_keys//target_files/${FZIP}-factory.zip\n"
cd /ssd2/androidsource/do-not-touch/axp/Build/LineageOS-20.0
...
echo -e "\t${FPID:32:16}"
echo -e "\t${FPID:48:16}\n"
out/host/linux-x86/bin/avbtool info_image --image out/target/product/sunfish/vbmeta.img 2> /dev/null || true
out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/sunfish/vbmeta.img 2> /dev/null \
    || (out/host/linux-x86/bin/avbtool info_image --image out/target/product/sunfish/obj/PACKAGING/target_files_intermediates/lineage_sunfish-target_files-eng.emy/IMAGES/vbmeta.img;\
        out/host/linux-x86/bin/avbtool verify_image --follow_chain_partitions --image out/target/product/sunfish/obj/PACKAGING/target_files_intermediates/lineage_sunfish-target_files-eng.emy/IMAGES/vbmeta.img)

echo -e "\n\nSUCCESS:\nYou should find your build here:\n/ssd/slim/zips/sunfish/LineageOS-20.0/release_keys/\n${ZIPINF}"
